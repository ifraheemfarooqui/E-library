...
export default function MyBooks() {
  ...
  const BookCard = ({ item, i }) => {
    ...
    return (
      <div onClick={() => navigate(`/reader/${book.id}`)}
        className="flex items-center gap-3 sm:gap-4 bg-white/4 border border-white/8 rounded-xl p-3 sm:p-4 cursor-pointer hover:border-[#6c5ce7]/40 transition group">
        <div className="w-12 h-16 sm:w-14 sm:h-20 rounded-lg flex items-center justify-center text-xl flex-shrink-0 overflow-hidden group-hover:-translate-y-0.5 transition-transform"
        ...
        <span className="text-[#9b8fc0] group-hover:text-white transition text-lg flex-shrink-0">›</span>
      </div>
    );
  };
  ...
  return (
    <Layout title="My Books">
      ...
      <p className="text-[#9b8fc0] text-sm mb-6 sm:mb-8">Your reading list and progress</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 sm:py-24 text-[#9b8fc0]">
          <div className="text-5xl mb-4 opacity-30">📚</div>
          <p className="mb-2">No books yet</p>
          <p className="text-sm mb-4">Go to the Library and start reading!</p>
